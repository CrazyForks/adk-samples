...
from google.api_core.exceptions import GoogleAPICallError, GoogleAPIError, RetryError
...
def _validate_bucket_name(bucket_name: str) -> Dict[str, Any]:
    ...
    if (
        len(bucket_name) < MIN_BUCKET_NAME_LENGTH
        or len(bucket_name) > MAX_BUCKET_NAME_LENGTH
    ):
        return {
            "valid": False,
            "message": f"Bucket name must be between {MIN_BUCKET_NAME_LENGTH} and {MAX_BUCKET_NAME_LENGTH} characters",
        }

    if not GCS_BUCKET_NAME_PATTERN.match(bucket_name):
        return {
            "valid": False,
            "message": "Bucket name can only contain lowercase letters, numbers, hyphens, and underscores",
        }
...
def _get_gcs_client() -> storage.Client:
    ...
    try:
        ...
        raise Exception(
            "Network timeout while connecting to Google Cloud. Check your internet connection or try again."
        )
    except Forbidden:
        raise Exception(
            "Access denied. Your credentials do not have permission to access this project."
        )
...
def create_gcs_bucket(
    bucket_name: str, location: str = "us-central1", storage_class: str = "STANDARD"
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {
            "status": "error",
            "message": f"Bucket name '{bucket_name}' is already taken by another user",
        }
    except Exception as e:
...
def upload_directory_to_gcs(
    ...
    try:
        ...
        if not os.path.isdir(local_path):
            return {
                "status": "error",
                "message": f"Path is not a directory: {local_path}",
            }
        ...
        for root, dirs, files in os.walk(local_path):
            for file in files:
                ...
                if gcs_prefix:
                    gcs_object_name = (
                        f"{gcs_prefix.rstrip('/')}/{relative_path.replace(os.sep, '/')}"
                    )
                else:
                ...
                try:
                    ...
                    blob.upload_from_filename(
                        local_file_path, timeout=60
                    )  # Set a timeout for upload
                    ...
                    failed_uploads.append(
                        {"local_path": local_file_path, "error": "Upload timed out"}
                    )
                except GoogleAPIError as gcs_error:
                    failed_uploads.append(
                        {
                            "local_path": local_file_path,
                            "error": f"GCS API error: {str(gcs_error)}",
                        }
                    )
                except Exception as e:
                    failed_uploads.append(
                        {"local_path": local_file_path, "error": str(e)}
                    )
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return {
                    "status": "error",
                    "message": "Upload of zip file to GCS timed out.",
                }
...
def download_from_gcs(
    bucket_name: str, object_name: str, local_path: str
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {"status": "error", "message": f"Google Cloud error occurred: {str(e)}"}
    except Exception as e:
        return {
            "status": "error",
...
def delete_from_gcs(
    bucket_name: str, object_name: Optional[str] = None, prefix: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if object_name:
            ...
            return {
                "status": "error",
                "message": "Either object_name or prefix must be specified",
            }
...
def list_gcs_buckets() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "error", "message": f"Google API error occurred: {str(e)}"}
    except Exception as e:
        return {
            "status": "error",
...
def list_gcs_objects(bucket_name: str, prefix: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": "error",
            "message": f"Access denied to bucket '{bucket_name}'. Check your permissions.",
        }
    except (RequestsTimeout, socket.timeout):
        return {
            "status": "error",
            "message": "Request timed out while listing GCS objects.",
        }
    except GoogleAPIError as e:
        ...
        return {
            "status": "error",
            "message": f"Unexpected error while listing GCS objects: {str(e)}",
        }
...
def delete_gcs_bucket(bucket_name: str, force: bool = False) -> Dict[str, Any]:
    ...
    try:
            ...
            try:
                ...
                for blob in blobs:
                    try:
                        ...
                        return {
                            "status": "error",
                            "message": f"Failed to delete object '{blob.name}': {str(e)}",
                        }
            except Exception as e:
                return {
                    "status": "error",
                    "message": f"Failed to list/delete objects in bucket: {str(e)}",
                }
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Bucket '{bucket_name}' is not empty. Use force=True to delete all objects first.",
            }
        ...
        return {
            "status": "error",
            "message": f"Request timed out while deleting bucket '{bucket_name}'",
        }
    except GoogleAPIError as e:
        return {"status": "error", "message": f"Google API error: {str(e)}"}
    except Exception as e:
        return {
            "status": "error",
            "message": f"Unexpected error during bucket deletion: {str(e)}",
        }
