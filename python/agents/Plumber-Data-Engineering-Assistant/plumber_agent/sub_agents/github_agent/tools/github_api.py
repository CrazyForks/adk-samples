...
def authenticate_github(token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": "error",
            "message": "Request timed out. GitHub did not respond within the expected time.",
        }
    except requests.exceptions.HTTPError as e:
        if e.response.status_code == 401:
            return {
                "status": "error",
                "message": "Authentication failed (401): Bad credentials. Please check your token.",
            }
        return {
            "status": "error",
            "message": f"Authentication failed ({e.response.status_code}): {e.response.text}",
        }
...
def search_repositories(repo_name: str, token: str) -> Dict[str, Any]:
    ...
    try:
        response = requests.get(
            ...
            params={"q": repo_name},
            timeout=10,
        )  # add timeout to avoid hanging else use line 43
        response.raise_for_status()
        ...
        return {
            "status": "error",
            "message": f"Search failed ({e.response.status_code}): {e.response.text}",
        }
    except Exception as e:
        return {
            "status": "error",
...
def list_branches(repository: str, token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if e.response.status_code == 404:
            return {
                "status": "error",
                "message": "Could not list branches (404): Repository not found.",
            }
        return {
            "status": "error",
            "message": f"Failed to list branches ({e.response.status_code}): {e.response.text}",
        }
    except Exception as e:
        return {
            "status": "error",
            "message": f"An error occurred while listing branches: {str(e)}",
        }
...
def download_repository(
    ...
    try:
        ...
        if upload_to_gcs:
            ...
            if not gcs_bucket_name and repo is not None:
        ...
        if e.response.status_code == 404:
            return {
                "status": "error",
                "message": "Download failed (404): Repository or branch not found.",
            }
        return {
            "status": "error",
            "message": f"Download failed ({e.response.status_code}): {e.response.text}",
        }
    except Exception as e:
        return {
            "status": "error",
...
def download_repository_to_gcs(
    repository: str,
    token: str,
    branch: str = "main",
    gcs_bucket_name: Optional[str] = None,
    ...
    try:
        ...
        return {
            "status": "error",
            "message": "Request timed out. GitHub server did not respond in time.",
        }
    except requests.exceptions.HTTPError as e:
        if e.response.status_code == 404:
            return {
