...
def get_transformation_sql(gcs_url: str) -> dict:
    """
    ...
    Parameters:
        gcs_url (str): The GCS URL pointing to the input file (e.g., "gs://bucket_name/path/to/file.csv").
...
def get_dataproc_template(user_prompt: str, language: LANGUAGE_OPTIONS):
    ...
    searches for README files related to available templates, and uses a matching mechanism
    to find the template that best fits the user prompt.

    Parameters:
        user_prompt (str): A natural language description of the desired data processing task.
        language (LANGUAGE_OPTIONS): The programming language to filter templates by (e.g., "Python" or "Java").
    ...
    Returns:
        dict or str:
            ...
            - On exception: A string with the error message.

    Exceptions:
        Returns the exception message as a string if an error occurs during execution.
    """
    try:
        ...
        if status.get("repo_path"):
            ...
            if language == "Python":
                readme_files = find_files(
                    dir=f"{status['repo_path']}/python/dataproc_templates/",
                    regex="README.md",
                )
            elif language == "Java":
                readme_files = find_files(
                    dir=f"{status['repo_path']}/java/src/main/java/com/google/cloud/dataproc/templates",
                    regex="README.md",
                )
...
def run_dataproc_template(
    ...
    template (in Python or Java), and optionally modifies the template to include
    transformation SQL logic. It returns the result of the job execution.
    ...
    try:
        ...
        if language.upper() == "JAVA":
            template_bin_path = JAVA_TEMPLATE_START_SCRIPT_BIN_PATH  # FOLDER LOCATION WHERE THE BIN FOLDER IS PRESENT TO RUN start.sh
        else:
