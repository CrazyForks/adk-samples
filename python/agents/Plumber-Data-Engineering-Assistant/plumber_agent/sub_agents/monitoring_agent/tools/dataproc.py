...
def get_dataproc_cluster_logs_with_name(
    project_id: str, dataproc_cluster_name: str = "", _limit: int = 10
) -> dict:
    """
...
def get_dataproc_cluster_logs_with_id(
    project_id: str, dataproc_cluster_id: str, _limit: int = 10
) -> dict:
    """
...
def get_dataproc_normal_job_logs_with_id(
    project_id: str, region: str, job_id: str, filter: str
) -> Union[subprocess.CompletedProcess, List]:
    """
    ...
    Args:
        ...
        filter (str, required): The gcloud dataproc subcommand part ('jobs' or 'batches').
    ...
    try:
        ...
        print(f"Result for '{filter}' command: returncode={result.returncode}")
        return result
    except FileNotFoundError:
        logger.warning(
            "gcloud command not found. Please ensure it is installed and in your system's PATH."
        )
        print(
            "gcloud command not found. Please ensure it is installed and in your system's PATH."
        )
        ...
        logger.error(f"An error occurred while calling command: {str(e)}")
        print(f"An error occurred: {e}")
        return []
...
def get_dataproc_job_logs_with_id(
    ...
    Returns:
        dict: A dictionary containing the status, report, and raw command output details.
    """
    ...
    try:
        ...
        # gcloud returns non-zero code if not found or failed, but we only fall back if we suspect "not found"
        # We assume a failure to find the batch results in a non-zero code and a message in stderr
        if (
        ...
        if result.returncode == 0:
            ...
            report = (
                f"Successfully waited for Dataproc {command_type} **{job_id}** to complete (gcloud exit code 0).\n\n"
                f"--- Log Output (stdout) ---\n{result.stdout}\n\n"
            ...
            report = (
                f"Dataproc {command_type} **{job_id}** failed, was not found, or the 'gcloud wait' command timed out (gcloud exit code {result.returncode}).\n\n"
        ...
        logger.error(f"An overall error occurred: {str(e)}")
